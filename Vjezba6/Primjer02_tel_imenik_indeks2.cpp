//Telefonski imenik: indeksna datoteka (2)
#include <iostream>
#include <fstream>
#include <stdlib.h>
#include <string.h>
...
void preslozi_indeks(char naziv[], tindeks spremi){
  tindeks indeks,pom;
  ...
  while(1){
    ...
    if (ind.eof())break;
    if ((indeks.tel_broj>spremi.tel_broj)&&(nadjeno==0)){
      nadjeno=1;
      ind.seekp((int)ind.tellg()-sizeof(tindeks));
      ind.write ((char *)&spremi,sizeof(tindeks));
      memcpy (&pom,&indeks,sizeof(tindeks));//spremanje tekuæeg zapisa u pom
    }else if (nadjeno==1){
      ind.seekp((int)ind.tellg()-sizeof(tindeks));
...
void unos_zapisa_rucno(){
  ...
  dat.close();dat.clear();
  ind.close();ind.clear();
  preslozi_indeks((char *)"imenik.ind",indeks);
};//unos_zapisa_rucno
...
void pretrazivanje_zapisa(int tel_broj){
  ...
  dat.open ("imenik.dat",ios::in|ios::binary);
  if (!dat){
    cout << "Datoteka nije ni kreirana!"<<endl;
    return;
  } else { // datoteka postoji
    dat.seekg (0, ios::end);
    N=dat.tellg()/sizeof(timenik);
    dat.close();dat.clear();
  }
  dat.open ("imenik.dat",ios::in|ios::binary);
  ind.open ("imenik.ind",ios::in|ios::binary);
...
int main(){
  ...
  do{cout << endl;
    cout << "0. brisanje sadrzaja datoteka" << endl;
